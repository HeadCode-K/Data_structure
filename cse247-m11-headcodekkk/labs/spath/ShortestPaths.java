...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	while(pq.isEmpty() == false) {
    	      PQEntry<VertexAndDist, Integer> updateHandle = pq.extractMin();
    	      VertexAndDist U = updateHandle.getElement();
    	      Vertex vtxn = U.getVertex();
    	      
    	      for(Edge edgi : vtxn.edgesFrom()) {
    	       int len = weights.get(edgi);
    	       int alt = U.getDistance() + len;
    	       
    	       PQEntry<VertexAndDist, Integer> vHandle = handles.get(edgi.to);
    	       VertexAndDist VD = vHandle.getElement(); 
    	       int dist = VD.getDistance();
    	       Vertex Vertex = VD.getVertex();

    	       if(alt < dist) {
    	        parentEdges.put(Vertex, edgi);
    	        VD.setDistance(alt);
