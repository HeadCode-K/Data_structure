...
public class AVLTree<T extends Comparable<T>> {
	...
	private void updateHeight(TreeNode<T> root) {
		...
		int heightL = (root.left == null ? -1 : root.left.height);
		int heightR = (root.right == null ? -1 : root.right.height);
	...
	private TreeNode<T> rebalance(TreeNode<T> root) {
		...
		int rootFactor = getBalance(root);
		...
		if (rootFactor == -2){
				...
				if (root.left != null) updateHeight (root. left);
				if (root.right != null) updateHeight (root.right);
				updateHeight(root) ;
				return root;
	}
