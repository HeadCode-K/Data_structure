...
public class MinHeap<E, P extends Comparable<P>> implements MinPriorityQueue<E,P> {
	...
	private void swap(int i, int j) {
		// TODO// Remember to update the Entries to reflect their new positions.

		
		HeapPQEntry index_j=entries.get(j); 
		int position_j = index_j.position;
		HeapPQEntry index_i =entries.get(i);
		int position_i=index_i.position;

		index_i.position = position_j;
		index_j.position = position_i;
		entries.set(j,index_i); 
		entries.set(i,index_j);
		ticker.tick(4);
	...
	public PQEntry<E, P> insert(E thing, P priority) {
		...
		// 1.new item
		HeapPQEntry newNode = new HeapPQEntry(thing,priority);
		entries.add(newNode);
		newNode.position = entries.indexOf(newNode);
		ticker.tick(2);
	...
	public PQEntry<E, P> extractMin() {
		...
		HeapPQEntry rootNode = entries.get(0);
	 ...
	 */
	//Function that maintains the min heap property after insertion and extract min
	...
	private void bubbleUp(int startIndex) {
		// TODO
		HeapPQEntry newNode = entries.get(startIndex);
		// tick
		ticker.tick();
		...
		if(startIndex == 0||entries.size()==0) {
			...
			HeapPQEntry parent01 = entries.get(parentIndex);
			// tick
			ticker.tick(2);
		
		
		if (newNode.getPriority().compareTo(parent01.getPriority())<0) {
